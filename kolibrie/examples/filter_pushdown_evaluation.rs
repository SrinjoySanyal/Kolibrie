...
fn append_alarm_fanout_triples(data: &mut String, machine_id: usize) {
    for alarm_idx in 0..3 {
        ...
        for action_idx in 0..2 {
            ...
            data.push_str(action_uri.as_str());
        }
    }
}
...
fn evaluate_query(filteringQuery: &str) -> PolarsResult<()> {
    ...
    let (card1000_s, intel1000_s, dumb1000_s) = runQuery(db_1000, filteringQuery, 1000);
    let mut df_1000_s = df![
        "cardinality" => card1000_s,
        "intelligent runtime" => &intel1000_s,
        "dumb runtime" => &dumb1000_s
    ]?;
    let mut file1000_st = File::create(format!("filtering1000.csv")).expect("could not create file");
    ...
    let (card2000_s, intel2000_s, dumb2000_s) = runQuery(db_2000, filteringQuery, 2000);
    let mut df_2000_s = df![
        "cardinality" => card2000_s,
        "intelligent runtime" => &intel2000_s,
        "dumb runtime" => &dumb2000_s
    ]?;
    let mut file2000_st = File::create(format!("filtering2000.csv")).expect("could not create file");
    ...
    let (card3000_s, intel3000_s, dumb3000_s) = runQuery(db_3000, filteringQuery, 3000);
    let mut df_3000_s = df![
        "cardinality" => card3000_s,
        "intelligent runtime" => &intel3000_s,
        "dumb runtime" => &dumb3000_s
    ]?;
    let mut file3000_st = File::create(format!("filtering3000.csv")).expect("could not create file");
    ...
    let (card4000_s, intel4000_s, dumb4000_s) = runQuery(db_4000, filteringQuery, 4000);
    let mut df_4000_s = df![
        "cardinality" => card4000_s,
        "intelligent runtime" => &intel4000_s,
        "dumb runtime" => &dumb4000_s
    ]?;
    let mut file4000_st = File::create(format!("filtering4000.csv")).expect("could not create file");
    ...
    let (card5000_s, intel5000_s, dumb5000_s) = runQuery(db_5000, filteringQuery, 5000);
    let mut df_5000_s = df![
        "cardinality" => card5000_s,
        "intelligent runtime" => &intel5000_s,
        "dumb runtime" => &dumb5000_s
    ]?;
    let mut file5000_st = File::create(format!("filtering5000.csv")).expect("could not create file");
    ...
    let (card6000_s, intel6000_s, dumb6000_s) = runQuery(db_6000, filteringQuery, 6000);
    let mut df_6000_s = df![
        "cardinality" => card6000_s,
        "intelligent runtime" => &intel6000_s,
        "dumb runtime" => &dumb6000_s
    ]?;
    let mut file6000_st = File::create(format!("filtering6000.csv")).expect("could not create file");
    ...
    let (card7000_s, intel7000_s, dumb7000_s) = runQuery(db_7000, filteringQuery, 7000);
    let mut df_7000_s = df![
        "cardinality" => card7000_s,
        "intelligent runtime" => &intel7000_s,
        "dumb runtime" => &dumb7000_s
    ]?;
    let mut file7000_st = File::create(format!("filtering7000.csv")).expect("could not create file");
    ...
    let (card8000_s, intel8000_s, dumb8000_s) = runQuery(db_8000, filteringQuery, 8000);
    let mut df_8000_s = df![
        "cardinality" => card8000_s,
        "intelligent runtime" => &intel8000_s,
        "dumb runtime" => &dumb8000_s
    ]?;
    let mut file8000_st = File::create(format!("filtering8000.csv")).expect("could not create file");
    ...
    let (card9000_s, intel9000_s, dumb9000_s) = runQuery(db_9000, filteringQuery, 9000);
    let mut df_9000_s = df![
        "cardinality" => card9000_s,
        "intelligent runtime" => &intel9000_s,
        "dumb runtime" => &dumb9000_s
    ]?;
    let mut file9000_st = File::create(format!("filtering9000.csv")).expect("could not create file");
    ...
    let (card10000_s, intel10000_s, dumb10000_s) = runQuery(db_10000, filteringQuery, 10000);
    let mut df_10000_s = df![
        "cardinality" => card10000_s,
        "intelligent runtime" => &intel10000_s,
        "dumb runtime" => &dumb10000_s
    ]?;
    let mut file10000_st = File::create(format!("filtering10000.csv")).expect("could not create file");
    ...
    let mut plotfile = File::create("FilterPDResults.csv").expect("could not create file");
...
fn runQuery(db: &mut SparqlDatabase, filteringQuery: &str, machines: u128) -> (Vec<u128>, Vec<u128>, Vec<u128>) {
    ...
    let output = parse_sparql_query(filteringQuery);
    let (
        _,
        (
            insert_clause,
    ...
    let filter_lo = build_logical_plan_filter(
        selected_variables
                .iter()
                .map(|(t, v)| (t.as_str(), v.as_str()))
                .collect(), 
    ...
    let intell_po = optimizer.find_best_plan(&filter_lo, db);
...
fn main() -> PolarsResult<()> {
    ...
    PREFIX mls: <http://www.w3.org/ns/mls#>
    PREFIX ext: <http://www.thesisextension.org/runml#>
    SELECT ?machine ?humid ?temp ?energy ?vibr ?alarm ?prediction WHERE {
